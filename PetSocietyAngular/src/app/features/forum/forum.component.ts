...
export class ForumComponent implements OnInit {
  ...
  submitComment(): void {
    ...
    this.forumService.addComment(payload).subscribe({
      next: (newComment: any) => {
        // 更新 Modal 內的文章資料
        if (this.editingArticle) { // 確保 editingArticle 不為 null
          this.editingArticle.commentCount++; // 留言數 +1
          this.updatePopularScore(this.editingArticle); // 重新計算熱門度

          // 同步回主列表 (recentPosts)
          const postInList = this.recentPosts.find(p => p.articleId === this.editingArticle?.articleId); // 找到對應文章
          if (postInList) { // 確保找到了文章
            postInList.commentCount = this.editingArticle.commentCount; // 更新留言數
            postInList.popular = this.editingArticle.popular; // 更新熱門度
          }
        }
        ...
        this.editingArticle!.popular = this.calculateLocalPopular(
          this.editingArticle!.like || 0,
          this.editingArticle!.disLike || 0,
          this.editingArticle!.commentCount
        );
        ...
        if (postInList) {
          ...
          postInList.commentCount = this.editingArticle!.commentCount; // 確保留言次數同步
          postInList.popular = this.editingArticle!.popular; // 確保熱門度同步
        }
        this.commentForm.reset(); // 清空留言表單
        console.log('回覆成功！');
      },
      error: (err) => {
  ...
  private updatePopularScore(article: Article) {
    const l = article.like || 0;
    const d = article.disLike || 0;
    const c = article.commentCount || 0;

    // 套用公式：(l * 2) + (c * 3) - (d * 2)
    article.popular = Math.max(0, (l * 2) + (c * 3) - (d * 1));
  }

  // 當彈窗數據變動時，同步更新表格列表中的數據
  ...
  private calculateLocalPopular(like: number, dislike: number, commentCount: number): number {
    // 這裡套用與後端 CalculatePopular 方法完全相同的公式
    // score = (l * 2) + (c * 3) - (d * 2)
    const score = (like * 2) + (commentCount * 3) - (dislike * 2);
  ...
  closeModal(): void {
    // 找出列表中的該篇文章，確保它的數據與剛才編輯/檢視時一致
    if (this.editingArticle) {// 同步更新列表中的留言數與熱門度
      const postInList = this.recentPosts.find(a => a.articleId === this.editingArticle!.articleId);
      if (postInList) {// 更新留言數與熱門度
        postInList.commentCount = this.editingArticle.commentCount;
        postInList.popular = this.editingArticle.popular;
      }
    }

    this.showModal = false; // 隱藏彈窗
    this.imagePreview = null; // 清空圖片預覽
    this.selectedFile = null; // 清空選擇的檔案
    this.articleForm.reset({ articleId: 0 }); // 重置表單
  }
